...
public:
	...
	/** Invalid handle */
	static const uint64 InvalidDataHandle = MAX_uint64;
	 ...
	 * @param FixedMode If true and InBlocks * InColumns is nonzero, allocation happens immediately and it remains static
	 ...
	 * @param Column index
	 */
	FStructDatablock::DatablockPtr GetDatablockPtr(uint32 InRow, uint32 InColumn);

	/**
	 * Retrieve the data associated with the provided handle
	 * @param Handle A handle to the requested data
	 */
	FStructDatablock::DatablockPtr GetDatablockPtr(DataHandle Handle);

	/**
	 * Retrieve the data associated with the provided row
	 * @param InRow Row index
	 */
	void GetDatablockPtrsForRow(uint32 InRow, TArray<FStructDatablock::DatablockPtr>& OutDataBlocks);
...
private:
	 ...
	 * @param Minimum block capacity in dynamic mode, exact capacity in fixed mode
	 */
	 ...
	 */
	FORCEINLINE Row* GetRow(uint32 RowIndex)
	{
		check(RowIndex < (uint32) Rows.Num());
	 ...
	 */
	DataHandle MakeHandle(uint32 DatablockIndex, FStructDatablock::DatablockHandle DatablockHandle);
	 ...
	 * @param Handle Handlee to separate.
	 * @param OutDatablockIndex Index to the datablock containing the memory allocation for this handle
	 * @param Out DatablockHandle Handle pointing to a memory location inside of a datablock
	 */
	bool GetHandleData(DataHandle Handle, uint32& OutDatablockIndex, FStructDatablock::DatablockHandle& OutDatablockHandle);
